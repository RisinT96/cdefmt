...
int main(int argc, char* cargv[]) {
  for (size_t i = 0; i < 100000; i++) {
    ...
    CDEFMT_INFO("Named parameters: {some_f32} {some_struct.b} {1} {some_u16} {}", some_bool,
    ...
    char* really_long_string =
        ...
        "Captain Elias sat in the dim cockpit, staring at the blinking distress beacon. It had "
        "been "
        "\n"
        ...
        "\"This is Captain Elias of the Aurora. If anyone is out there... I won't last much "
        "longer.\n"
...
void cdefmt_log(const void* id, size_t size, enum cdefmt_level level, size_t count, ...) {
  ...
  if (S_ISFIFO(stat.st_mode)) {
    ...
    for (size_t i = 0; i < count; i++) {
      ...
      void const* arg_ptr = va_arg(args, void const*);
      void const* actual_arg_ptr = arg_ptr;

      if (CDEFMT_IS_ARG_BY_VALUE(arg_size)) {
        // Actually point to self, as I was passed bu value....
        actual_arg_ptr = &arg_ptr;
        arg_size &= ((1ULL << ((sizeof(arg_size) * CHAR_BIT) - 1)) - 1);
      }

      fwrite(actual_arg_ptr, arg_size, 1, stdout);
    }
  ...
  printf("level: %u, id: %#010lx, size: %-3zu data: [", level, (uintptr_t)id, size);
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    for (size_t arg_i = 0; arg_i < arg_size; arg_i++) {
      printf("%02x", ((const uint8_t*)actual_arg_ptr)[arg_i]);
      if (arg_i + 1 < arg_size) {
        printf(", ");
      }
    }
